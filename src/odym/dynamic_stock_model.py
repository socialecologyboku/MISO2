...
def __version__():
    ...
    return str('1.0'), str(
        'Class DynamicStockModel, dsm. Version 1.0. Last change: July 25th, 2019. '
        'Check https://github.com/IndEcol/ODYM for latest version.')
...
class DynamicStockModel:
    ...
    __slots__ = "t", "i", "s", "s_c", "o", "o_c", "lt", "name", "pdf", "sf"
    ...
    def check_stock_balance(self):
        """ Check wether inflow, outflow, and stock are balanced. If possible,
        the method returns the vector 'Balance', where Balance = inflow - outflow - stock_change"""
        try:
    ...
    def compute_outflow_pdf(self):
        ...
        if self.pdf is None:
            self.compute_sf()
            # computation of pdfs moved to this method: compute survival functions sf first,
            # then calculate pdfs from sf.
    ...
    def compute_sf(self):  # survival functions
        ...
        The method compute outflow_sf returns an array year-by-cohort of the surviving fraction of a flow added to stock
        ...
        The method does nothing if the sf alreay exists. For example, sf could be assigned to the dynamic stock model
        ...
        if self.sf is None:
            ...
            if self.lt['Type'] == 'Fixed':
                # fixed lifetime, age-cohort leaves the stock in the model year when the age specified as 'Mean' is
                # reached.
                for m in range(0, len(self.t)):  # cohort index
                    self.sf[m::, m] = np.multiply(1, (
                            np.arange(0, len(self.t) - m) < self.lt['Mean'][m]))  # converts bool to 0/1
            ...
            if self.lt['Type'] == 'Normal':
                ...
                for m in range(0, len(self.t)):  # cohort index
                    if self.lt['Mean'][m] != 0:  # For products with lifetime of 0, sf == 0
                        self.sf[m::, m] = scipy.stats.norm.sf(np.arange(0, len(self.t) - m), loc=self.lt['Mean'][m],
                                                              scale=self.lt['StdDev'][m])
                        # NOTE: As normal distributions have nonzero pdf for negative ages, which are physically
                        # impossible,
                        # these outflow contributions can either be ignored (violates the mass balance) or
                        # allocated to the zeroth year of residence, the latter being implemented in the method
                        # compute compute_o_c_from_s_c.
                        # As alternative, use lognormal or folded normal distribution options.

            if self.lt['Type'] == 'FoldedNormal':
                # Folded normal distribution, cf. https://en.wikipedia.org/wiki/Folded_normal_distribution
                for m in range(0, len(self.t)):  # cohort index
                    if self.lt['Mean'][m] != 0:  # For products with lifetime of 0, sf == 0
                        self.sf[m::, m] = scipy.stats.foldnorm.sf(np.arange(0, len(self.t) - m),
                                                                  self.lt['Mean'][m] / self.lt['StdDev'][m], 0,
                                                                  scale=self.lt['StdDev'][m])
                        # NOTE: call this option with the parameters of the normal distribution mu and sigma of
                        # curve BEFORE folding,
            ...
            if self.lt['Type'] == 'LogNormal':  # lognormal distribution
                ...
                # not those of the underlying normal distribution, need to be specified! conversion of
                # parameters done here:
                for m in range(0, len(self.t)):  # cohort index
                    if self.lt['Mean'][m] != 0:  # For products with lifetime of 0, sf == 0
                        # calculate parameter mu    of underlying normal distribution:
                        LT_LN = np.log(self.lt['Mean'][m] / np.sqrt(1 + self.lt['Mean'][m] * self.lt['Mean'][m] / (
                                self.lt['StdDev'][m] * self.lt['StdDev'][m])))
                        # calculate parameter sigma of underlying normal distribution:
                        SG_LN = np.sqrt(np.log(1 + self.lt['Mean'][m] * self.lt['Mean'][m] / (
                                self.lt['StdDev'][m] * self.lt['StdDev'][m])))
                        # compute survial function
                        self.sf[m::, m] = scipy.stats.lognorm.sf(np.arange(0, len(self.t) - m), s=SG_LN, loc=0,
                                                                 scale=np.exp(LT_LN))
            ...
            if self.lt['Type'] == 'MISO2LogNormal':
                # simplified parametrisation used in MISO2 code.
                for m in range(0, len(self.t)):  # cohort index
                    if self.lt['Mean'][m] != 0:  # For products with lifetime of 0, sf == 0
                        self.sf[m::, m] = scipy.stats.lognorm.sf(np.arange(0, len(self.t) - m), s=self.lt["StdDev"][m], loc=0,
                                                                 scale=self.lt["Mean"][m])
            if self.lt['Type'] == 'Weibull':
                ...
                for m in range(0, len(self.t)):  # cohort index
                    if self.lt['Shape'][m] != 0:  # For products with lifetime of 0, sf == 0
                        self.sf[m::, m] = scipy.stats.weibull_min.sf(np.arange(0, len(self.t) - m),
                                                                     c=self.lt['Shape'][m], loc=0,
        ...
        if self.s_c is not None:
            if self.o_c is None:
                ...
                self.o_c[np.diag_indices(len(self.t))] = self.i - np.diag(
                    self.s_c)  # allow for outflow in year 0 already
                return self.o_c
            else:
    ...
    def compute_evolution_initialstock(self, InitialStock, SwitchTime):
        ...
        if self.lt is not None:
            ...
            self.s_c[SwitchTime::, 0:SwitchTime] = np.tile(InitialStock.transpose(),
                                                           (len(self.t) - SwitchTime, 1)) * self.sf[SwitchTime::,
                                                                                            0:SwitchTime] / np.tile(
                Shares_Left, (len(self.t) - SwitchTime, 1))
        return self.s_c
        ...
        if self.s is not None:
            if self.lt is not None:
                ...
                for m in range(1, len(self.t)):  # for all years m, starting in second year
                    ...
                    self.o_c[m, 0:m] = self.s_c[m - 1, 0:m] - self.s_c[m,
                                                              0:m]  # outflow table is filled row-wise, for each year m.
                    ...
                    if NegativeInflowCorrect is False:  # if no correction for negative inflows is made
                        if self.sf[m, m] != 0:  # Else, inflow is 0.
                            self.i[m] = (self.s[m] - self.s_c[m, :].sum()) / self.sf[
                                m, m]  # allow for outflow during first year by rescaling with 1/sf[m,m]
                    ...
                    if NegativeInflowCorrect is True:
                        ...
                        # this option allows to extract additional stock items.
                        # The negative inflow correction implemented here was developed in a joined effort by
                        # Sebastiaan Deetman and Stefan Pauliuk.
                        ...
                        if InflowTest < 0:  # if stock-driven model would yield negative inflow
                            ...
                            if self.s_c[m, :].sum() != 0:
                                ...
                                # Distribute gap equally across all cohorts (each cohort is adjusted by the same
                                # %, based on surplus with regards to the prescribed stock)
                                # Delta_percent is a % value <= 100%
                            else:
                                Delta_percent = 0  # stock in this year is already zero, method does not work
                                # in this case.
                            # correct for outflow and stock in current and future years
                            # adjust the entire stock AFTER year m as well, stock is lowered in year m, so
                            # future cohort survival also needs to decrease.
                            self.o_c[m, :] = self.o_c[m, :] + (self.s_c[m,
                                                               :] * Delta_percent)
                            # increase outflow according to the lost fraction of the stock, based on Delta_c
                            self.s_c[m::, 0:m] = self.s_c[m::, 0:m] * (
                                    1 - Delta_percent)
                            # shrink future description of stock from previous age-cohorts by factor Delta_percent in
                            ...
                            self.s_c[m::, m] = self.i[m] * self.sf[m::, m]
                            self.o_c[m, m] = self.i[m] * (1 - self.sf[m, m])
                            # NOTE: This method of negative inflow correction is only of of many plausible methods of
                            # increasing the outflow to keep matching stock levels.
                        # It assumes that the surplus stock is removed in the year that it becomes obsolete. Each
                        # cohort loses the same fraction.
                        # Modellers need to try out whether this method leads to justifiable results.
                        # In some situations it is better to change the lifetime assumption than using the
                        # NegativeInflowCorrect option.
    ...
    def compute_stock_driven_model_initialstock(self, InitialStock, SwitchTime, NegativeInflowCorrect=False):
        ...
        The extra parameter InitialStock is a vector that contains the age structure of the stock at the END of the year
         Switchtime -1 = t0.
        ***
        ...
        ***
        ...
        if self.s is not None:
            if self.lt is not None:
                ...
                # assign initialstock to stock-by-cohort variable at END OF YEAR SwitchTime (here -1,
                ...
                self.s_c[:, 0:SwitchTime - 1] = np.einsum('tc,c->tc', self.sf[:, 0:SwitchTime - 1],
                ...
                if NegativeInflowCorrect is False:
                    for m in range(SwitchTime - 1, len(self.t)):  # for all years m, starting at SwitchTime
                        ...
                        if self.sf[m, m] != 0:  # Else, inflow is 0.
                            self.i[m] = (self.s[m] - self.s_c[m, :].sum()) / self.sf[
                                m, m]  # allow for outflow during first year by rescaling with 1/sf[m,m]
                        # NOTE: The stock-driven method may lead to negative inflows, if the stock development
                        # is in contradiction with the lifetime model.
                        # In such situations the lifetime assumption must be changed, either by directly
                        # using different lifetime values or by adjusting the outflows,
                    ...
                    for m in range(SwitchTime - 1, len(self.t)):  # for all years m, starting at SwitchTime
                        ...
                        if InflowTest < 0:
                            ...
                            if self.s_c[m, :].sum() != 0:
                                ...
                                Delta_percent = 0
                                # stock in this year is already zero, method does not work in this case.
                            # correct for outflow and stock in current and future years
                            # adjust the entire stock AFTER year m as well, stock is lowered in year m, so future
                            # cohort survival also needs to decrease.
                            # print(InflowTest)
                            ...
                            # shrink future description of stock from previous age-cohorts by factor Delta_percent
    ...
    def compute_stock_driven_model_initialstock_typesplit(self, FutureStock, InitialStock, SFArrayCombined, TypeSplit):
        ...
        Indices:
          ...
          T: Switch time: DEFINED as first year where historic stock is NOT present, = last year where historic stock
           is present +1.
             Switchtime is calculated internally, by subtracting the length of the historic stock from the total model
             length.
          g: product type
            ...
            if self.lt is not None:
                ...
                for c in range(0, SwitchTime):  # for all historic age-cohorts til SwitchTime - 1:
                    for g in range(0, Ng):
                        if SFArrayCombined[SwitchTime - 1, c, g] != 0:
                            ...
                            # If survival function is 0 but initial stock is not, the data are inconsisent and need
                            # to be revised.
                            # For example, a safety-relevant device with 5 years fixed lifetime but a 10 year old
                            # device is present.
                ...
                for t in range(SwitchTime, Ntt):  # for all years t, starting at SwitchTime
                    ...
                    if t == SwitchTime:
                        ...
                        o_cg[t - SwitchTime, :, :] = s_cg[t - SwitchTime - 1, :, :] - s_cg[t - SwitchTime, :, :]
    ...
    def compute_stock_driven_model_initialstock_typesplit_negativeinflowcorrect(self, SwitchTime, InitialStock,
                                                                                SFArrayCombined, TypeSplit,
        ...
        Indices:
          ...
          T: Switch time: DEFINED as first year where historic stock is NOT present, = last year where historic stock
          is present +1.
             Switchtime must be given as argument. Example: if the first three age-cohorts are historic, SwitchTime is
        ...
        if self.s is not None:
            if self.lt is not None:
                ...
                NIC_Flags = np.zeros((Ntt, 1))
                # inflow flog for future years, will be set to calculated negative inflow value if negative inflow
                # occurs and is corrected for.
                ...
                for c in range(0, SwitchTime):  # for all historic age-cohorts til SwitchTime - 1:
                    for g in range(0, Ng):
                        ...
                        # If survival function is 0 but initial stock is not, the data are inconsisent and need to
                        # be revised.
                        # For example, a safety-relevant device with 5 years fixed lifetime but a 10 year old
                        # device is present.
                        # Such items will be ignored and break the mass balance.
                ...
                s_cg[:, 0:SwitchTime, :] = np.einsum('tcg,cg->tcg', SFArrayCombined[:, 0:SwitchTime, :],
                                                     i_g[0:SwitchTime, :])
                    ...
                    for m in range(SwitchTime, len(self.t)):  # for all years m, starting at SwitchTime
                        ...
                        for g in range(0, Ng):
                            if SFArrayCombined[m, m, g] != 0:  # Else, inflow is 0.
                                i_g[m, g] = TypeSplit[m, g] * i0_test / SFArrayCombined[
                                    m, m, g]  # allow for outflow during first year by rescaling with 1/sf[m,m]
                                # NOTE: The stock-driven method may lead to negative inflows, if the stock development
                                # is in contradiction with the lifetime model.
                                # In such situations the lifetime assumption must be changed, either by directly using
                                # different lifetime values or by adjusting the outlfows,
                                # cf. the option NegativeInflowCorrect in the method compute_stock_driven_model.
                                # 2) Add new inflow to stock and determine future decay of new age-cohort
                ...
                if NegativeInflowCorrect is True:
                    for m in range(SwitchTime, len(self.t)):  # for all years m, starting at SwitchTime
                        ...
                        if i0_test < 0:
                            ...
                            # adjust the entire stock AFTER year m as well, stock is lowered in year m, so future
                            # cohort survival also needs to decrease.
                            o_cg[m, :, :] = o_cg[m, :, :] + (s_cg[m, :, :] * Delta_percent).copy()
                            # increase outflow according to the lost fraction of the stock, based on Delta_c
                            s_cg[m::, 0:m, :] = s_cg[m::, 0:m, :] * (1 - Delta_percent.copy())
                            # shrink future description of stock from previous age-cohorts by factor Delta_percent
                            # in current AND future years.
                            ...
                            for g in range(0, Ng):
                                if SFArrayCombined[m, m, g] != 0:  # Else, inflow is 0.
                                    i_g[m, g] = TypeSplit[m, g] * i0_test / SFArrayCombined[
                                        m, m, g]  # allow for outflow during first year by rescaling with 1/sf[m,m]
                                    # NOTE: The stock-driven method may lead to negative inflows, if the stock
                                    # development is in contradiction with the lifetime model.
                                    # In such situations the lifetime assumption must be changed, either by directly
                                    # using different lifetime values or by adjusting the outlfows,
                                    # cf. the option NegativeInflowCorrect in the method compute_stock_driven_model.
                                    # 2) Add new inflow to stock and determine future decay of new age-cohort
                                s_cg[m::, m, g] = i_g[m, g] * SFArrayCombined[m::, m, g]
                                o_cg[m, m, g] = i_g[m, g] * (1 - SFArrayCombined[m, m, g])
