...
class Obj(object):
    ...
    """
    __slots__ = "Name", "ID", "UUID", "Aspects", "Dimensions"
    ...
    def __init__(self, Name=None, ID=None, UUID=None):
        ...
        self.Aspects = {'Time': 'Model time', 'Cohort': 'Age-cohort', 'OriginProcess': 'Process where flow originates',
                        'DestinationProcess': 'Destination process of flow',
                        'OriginRegion': 'Region where flow originates from',
                        'DestinationRegion': 'Region where flow is bound to', 'Good': 'Process, good, or commodity',
                        'Material': 'Material: ore, alloy, scrap type, ...',
                        'Element': 'Chemical element'}  # Define the aspects of the system variables
        self.Dimensions = {'Time': 'Time', 'Process': 'Process', 'Region': 'Region',
                           'Good': 'Process, good, or commodity', 'Material': 'Material: ore, alloy, scrap type, ...',
                           'Element': 'Chemical element'}  # Define the dimensions of the system variables
...
class Classification(Obj):
    ...
    """
    __slots__ = "Dimension", "Items", "IDs", "AdditionalProps"

    def __init__(self, Name=None, ID=None, UUID=None, Dimension=None, Items=None, IDs=None, AdditionalProperties=None):
        ...
        self.AdditionalProps = AdditionalProperties  # Like population for regions, element composition for alloys, ...
...
class MFAsystem(Obj):
    ...
    """
    __slots__ = "Time_Start", "Time_End", "Geogr_Scope", "Elements", "Unit", "ProcessList", \
        "FlowDict", "StockDict", "ParameterDict", "IndexTable", "Graphical"

    def __init__(self, Name, Time_Start, Time_End, Geogr_Scope, Unit, IndexTable, Elements, ProcessList, FlowDict,
                 StockDict, ParameterDict, Graphical=None, ID=None, UUID=None, ):
    ...
    def Initialize_FlowValues(self):
        ...
        for key in self.FlowDict:
            if self.FlowDict[key].Values is None:
                self.FlowDict[key].Values = np.zeros(tuple(
                    [len(self.IndexTable.set_index('IndexLetter').loc[x]['Classification'].Items) for x in
                     self.FlowDict[key].Indices.split(',')]))
    ...
    def Initialize_StockValues(self):
        ...
        for key in self.StockDict:
            if self.StockDict[key].Values is None:
                self.StockDict[key].Values = np.zeros(tuple(
                    [len(self.IndexTable.set_index('IndexLetter').loc[x]['Classification'].Items) for x in
                     self.StockDict[key].Indices.split(',')]))
    ...
    def Initialize_ParameterValues(self):
        ...
        for key in self.ParameterDict:
            if self.ParameterDict[key].Values is None:
                self.ParameterDict[key].Values = np.zeros(tuple(
                    [len(self.IndexTable.set_index('IndexLetter').loc[x]['Classification'].Items) for x in
                     self.ParameterDict[key].Indices.split(',')]))
    ...
    def Consistency_Check(self):
        ...
        for key in self.FlowDict:
            if tuple([len(self.IndexTable.set_index('IndexLetter').loc[x]['Classification'].Items) for x in
                      self.FlowDict[key].Indices.split(',')]) != self.FlowDict[key].Values.shape:
                raise ValueError(
    ...
    def Flow_Sum_By_Element(self, FlowKey):
        ...
        """
        return np.einsum(
            self.FlowDict[FlowKey].Indices.replace(',', '') + '->' + self.IndexTable.loc['Time'].IndexLetter +
            self.IndexTable.loc['Element'].IndexLetter, self.FlowDict[FlowKey].Values)
    ...
    def Stock_Sum_By_Element(self, StockKey):
        ...
        """
        return np.einsum(
            self.StockDict[StockKey].Indices.replace(',', '') + '->' + self.IndexTable.loc['Time'].IndexLetter +
            self.IndexTable.loc['Element'].IndexLetter, self.StockDict[StockKey].Values)
    ...
    def MassBalance(self, Element=None):
        ...
        """
        Bal = np.zeros(
            (len(self.Time_L), len(self.ProcessList), len(self.Elements)))  # Balance array: years x process x element:
        ...
        for key in self.StockDict:  # Add all stock changes to the mass balance
            if self.StockDict[key].Type == 1:
                Bal[:, self.StockDict[key].P_Res, :] -= self.Stock_Sum_By_Element(
                    key)  # 1: net stock change or addition to stock
    ...
    def SankeyExport(self, Year, Path,
                     Element):  # Export data for given year in excel format for the D3.js Circular Sankey method
        ...
        for m in range(0, len(self.ProcessList)):
            if self.ProcessList[m].Graphical is None:
                raise ValueError(
                    f'Graphical properties of process number {m} are not set. '
                    f'No export to Sankey possible, as position of process on canvas etc. needs is not specified.')
...
class Process(Obj):
    ...
    __slots__ = "Bipartite", "Extensions", "Graphical", "Parameters"
    ...
    def add_extension(self, Time=None, Name=None, Value=None, Unit=None,
                      Uncert=None):  # Extensions flows that are not part of the system-wide mass balance!
        if self.Extensions is None:
            self.Extensions = []
        self.Extensions.append(
            Flow(P_Start=self.ID, P_End=None, Time=Time, Name=Name, Unit=Unit, Value=Value, Uncert=Uncert))
...
class Flow(Obj):  # Flow needs to at least have dimension time x element
    ...
    __slots__ = "P_Start", "P_End", "Indices", "Values", "Uncert", "Unit", "Color"

    def __init__(self, Name=None, ID=None, UUID=None, P_Start=None, P_End=None, Indices=None, Values=None, Uncert=None,
                 Unit=None, Color=None):
...
class Stock(Obj):  # Flow needs to at least have dimension time x element
    ...
    __slots__ = "P_Res", "Indices", "Type", "Values", "Uncert", "Unit", "Color"

    def __init__(self, Name=None, ID=None, UUID=None, P_Res=None, Indices=None, Type=None, Values=None, Uncert=None,
                 Unit=None, Color=None):
...
class Parameter(Obj):
    ...
    """
    __slots__ = "P_Res", "Indices", "MetaData", "Values", "Values", "Uncert", "Unit"

    def __init__(self, Name=None, ID=None, UUID=None, P_Res=None, MetaData=None, Indices=None, Values=None, Uncert=None,
                 Unit=None):
