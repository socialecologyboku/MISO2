...
"""
import unittest
# import odym.ODYM_Functions as msf  # import the ODYM function file
import numpy as np
import pandas as pd
import odym.ODYM_Classes as msc  # import the ODYM class file
...
F_0_1_Values1 = np.array(
    [[1, 1], [0, 0], [2, 2], [1, 1], [4, 4]])  # Define some arbitrary inflow data for each year and element
F_1_2_Values1 = np.array([[1, 1], [1, 1], [32, 32], [1, 1], [4, 4]])  #
F_2_1_Values1 = np.array(
    [[0, 0], [1, 1], [30, 30], [0, 0], [0, 0]])  # Define flow back to process 1, make sure mass balance holds.
...
ModelClassification_Test1['Time'] = msc.Classification(Name='Time', Dimension='Time',
                                                       Items=[2010, 2011, 2012, 2013, 2014])
ModelClassification_Test1['Element'] = msc.Classification(Name='Chem_Elements', Dimension='Element',
                                                          Items=['All', 'Fe'])
...
Model_Aspects = ['Time', 'Element',
                 'Unity']  # aspects of the model to be considered. Must be the same as the items of the ModelClassification_Test1 dictionary
...
IndexTable = pd.DataFrame({'Aspect': Model_Aspects,  # 'Time' and 'Element' must be present!
                           ...
                           'IndexLetter': ['t', 'e',
                                           'i']})  # Unique one letter (upper or lower case) indices to be used later for calculations. t for time, e for element
...
MyMFA_Test1.FlowDict['F_1_2'] = msc.Flow(Name='Processed input', P_Start=1, P_End=2, Indices='t,e', Values=None,
                                         Uncert=None)
MyMFA_Test1.FlowDict['F_2_1'] = msc.Flow(Name='Sent back to 1', P_Start=2, P_End=1, Indices='t,e', Values=None,
                                         Uncert=None)
