...
def ListStringToListNumbers(ListStr):
    ...
    return [int(s) for s in
            ListStr[ListStr.find('['):ListStr.find(']') + 1].replace('[', ',').replace(']', ',').split(',') if
            s.isdigit()]
...
def ParseClassificationFile_Main(Classsheet, Mylog):
    ...
    while Classsheet.cell(1, ci).value != None:
        ...
        while Classsheet.cell(ri, ci).value != None:
            ...
            ri += 1
        MasterClassification[ThisName] = msc.Classification(Name=ThisName, Dimension=ThisDim, ID=ThisID, UUID=ThisUUID,
                                                            Items=TheseItems)
        ci += 1
...
def ParseConfigFile(Model_Configsheet, ScriptConfig, Mylog):
    ...
    while Model_Configsheet.cell(PLix + 1, 3).value != None:
        ...
        PL_IndexLayer.append(
            ListStringToListNumbers(Model_Configsheet.cell(PLix + 1, 8).value))  # strip numbers out of list string
        PLix += 1
...
def ReadParameter(ParPath, ThisPar, ThisParIx, IndexMatch, ThisParLayerSel, MasterClassification,
    ...
    if ParHeader.cell_value(ri, 1) == 'List':
            ...
            for mx in range(0, len(IList)):  # mx iterates over the aspects of the parameter
                ...
                try:
                    TargetPosition.append(
                        IndexTable.set_index('IndexLetter').loc[ThisParIx[mx]].Classification.Items.index(CurrentItem))
                except:
        ...
        Mylog.info(str(ValIns.sum()) + ' of ' + str(
            np.prod(IndexSizesM)) + ' values for parameter ' + ThisPar + ' were assigned.')

    ### Table version ###
    if ParHeader.cell_value(ri,
                            1) == 'Table':  # have 3 while loops, one for row indices, one for column indices, one for value layers
        ...
        if set(RIList).issubset(set(IndexTable_ClassificationNames)) is False:
            Mylog.error(
                'CLASSIFICATION ERROR: Row index list of data file for parameter ' + ThisPar + ' contains indices that are not part of the current model run.')
        if set(CIList).issubset(set(IndexTable_ClassificationNames)) is False:
            Mylog.error(
                'CLASSIFICATION ERROR: Column index list of data file for parameter ' + ThisPar + ' contains indices that are not part of the current model run.')
        ...
        for m in range(0, RowNos):
            ...
            for mc in range(0, len(RIList)):
                ...
                try:
                    ...
                    TPIX = IndexTable.set_index('IndexLetter').loc[RIIndexLetter[mc]].Classification.Items.index(
                        CurrentItem)
        ...
        for n in range(0, ColNos):
            ...
            for mc in range(0, len(CIList)):
                ...
                try:
                    ...
                    TPIX = IndexTable.set_index('IndexLetter').loc[CIIndexLetter[mc]].Classification.Items.index(
                        CurrentItem)
    ...
    if ParHeader.cell_value(ri, 1) == 'LIST':
        ...
        while True:
            ...
            if len(TargetPosition) == len(ThisParIx):
                Values[tuple(TargetPosition)] = CV
                ValIns[tuple(TargetPosition)] = 1
                Uncertainty[Tuple_MI(TargetPosition, IndexSizesM)] = ValuesSheet.cell_value(cx + RowOffset,
                                                                                            ColOffset + 3)
            cx += 1
    ...
    if ParHeader.cell_value(ri,
        ...
        for m in range(0, RowNos):
            ...
            for mc in range(0, len(RIList)):
                try:
                    CurrentItem = int(
                        ValuesSheet.cell_value(m + RowOffset, mc))  # in case items come as int, e.g., years
                except:
                    CurrentItem = ValuesSheet.cell_value(m + RowOffset, mc)
                try:
                    IX = ThisParIx.find(RIIndexLetter[mc])
        ...
        for m in range(0, RowNos):  # Read values from excel template
            for n in range(0, ColNos):
                ...
                if len(TargetPosition) == len(
                        ComIList):  # Read value if TargetPosition Tuple has same length as indexList
                    Values[tuple(TargetPosition)] = ValuesSheet.cell_value(m + RowOffset, n + ColOffset)
                    ValIns[tuple(TargetPosition)] = 1
                    # Add uncertainty
                    if ParseUncertainty == True:
                        ...
                        if 'Dataset_Uncertainty_Sheet' in MetaData:
                            Uncertainty[Tuple_MI(TargetPosition, IndexSizesM)] = UncertSheet.cell_value(m + RowOffset,
                                                                                                        n + ColOffset)
                cx += 1
...
def ReadParameterXLSX(ParPath, ThisPar, ThisParIx, IndexMatch, ThisParLayerSel, MasterClassification,
    ...
    if ParHeader.cell(ri, 2).value == 'LIST':  # ri = 21
        ...
        while True:
            ...
            if len(TargetPosition) == len(ThisParIx):
                Values[tuple(TargetPosition)] = CV
                ValIns[tuple(TargetPosition)] = 1
                Uncertainty[Tuple_MI(TargetPosition, IndexSizesM)] = ValuesSheet.cell(cx + RowOffset + 1,
                                                                                      ColOffset + 3).value
            cx += 1
    ...
    ### Table version ###
    if ParHeader.cell(ri,
                      2).value == 'TABLE':  # have 3 while loops, one for row indices, one for column indices, one for value layers
        ...
        if ParseUncertainty == True:
            if 'Dataset_Uncertainty_Sheet' in MetaData and Uncert_sheetname is None:
                UncertSheet = Parfile[MetaData['Dataset_Uncertainty_Sheet']]
                Mylog.info("2 Reading from UNCERTAINTY PAR SHEET: {} which is of type {}".format(
                    MetaData['Dataset_Uncertainty_Sheet'], type(UncertSheet)))
            elif Uncert_sheetname is not None and Uncert_sheetname in Parfile:
                UncertSheet = Parfile[Uncert_sheetname]
        ...
        for m in range(0, RowNos):
            ...
            for mc in range(0, len(RIList)):
                try:
                    CurrentItem = int(
                        ValuesSheet.cell(m + RowOffset + 1, mc + 1).value)  # in case items come as int, e.g., years
                except:
            ...
            for n in range(0, ColNos):
                ...
                if len(TargetPosition) == len(
                    ...
                    if ParseUncertainty == True:
                        if 'Dataset_Uncertainty_Global' in MetaData:
                            Uncertainty[Tuple_MI(TargetPosition, IndexSizesM)] = MetaData['Dataset_Uncertainty_Global']
                        if UncertSheet is not None:
                            Uncertainty[Tuple_MI(TargetPosition, IndexSizesM)] = UncertSheet.cell(m + RowOffset + 1,
                                                                                                  n + ColOffset + 1).value
                        # check why different index and why different offset
                cx += 1
